...
void NewProjectPanel::Layout()
{
		...
		ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings))
	{
		...
		if (ImGui::BeginTable("##SettingsTable", 2, ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_SizingFixedSame))
		{
			...
			ImGui::BeginDisabled(_model->_deformationType != DeformationType::PMVC && _model->_deformationType != DeformationType::PMVCO);
			{
				ImGui::TableNextRow();
				{
					ImGui::TableSetColumnIndex(0);
					...
					ImGui::TableSetColumnIndex(1);
					UIHelpers::SetRightAligned(100.0f);
					ImGui::InputScalar("##CubemapSize", ImGuiDataType_U64, &_model->_cubemapSize);
				}
				...
				ImGui::TableNextRow();
				{
					...
					ImGui::TableSetColumnIndex(1);
					UIHelpers::SetRightAligned(25.0f);
					ImGui::BeginDisabled(_model->_pmvcComputeType != PMVCComputeType::Cpu);
					ImGui::Checkbox("##PMVCInteriorDist", &_model->_pmvcUseInteriorDistance);
					ImGui::EndDisabled();
					ImGui::SameLine();
				}
			}
