...
void ProjectSettingsPanel::Layout()
{
		...
		ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings))
	{
		...
		if (ImGui::BeginTable("##SettingsTable", 2, ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_SizingFixedSame))
		{
			...
			ImGui::BeginDisabled(_modifiedProjectModel._deformationType != DeformationType::PMVC);
			{
			ImGui::TableNextRow();
			{
				ImGui::TableSetColumnIndex(0);
				...
				ImGui::TableSetColumnIndex(1);
				UIHelpers::SetRightAligned(125.0f);
				if (ImGui::BeginCombo("##PMVCComputeTypeSettings", ProjecSettingsHelpers::PMVCComputeMethodNames[static_cast<uint32_t>(_modifiedProjectModel._pmvcComputeType)], ImGuiComboFlags_HeightRegular))
				{
					for (auto i = 0; i < ProjecSettingsHelpers::PMVCComputeMethodNames.size(); i++)
					{
						const auto isSelected = (static_cast<uint32_t>(_modifiedProjectModel._pmvcComputeType) == i);
						if (ImGui::Selectable(ProjecSettingsHelpers::PMVCComputeMethodNames[i], isSelected))
						{
							_modifiedProjectModel._pmvcComputeType = static_cast<PMVCComputeType>(i);
						}
					}
					ImGui::EndCombo();
				}
			...
			ImGui::TableNextRow();
			{
				...
				ImGui::TableSetColumnIndex(1);
				UIHelpers::SetRightAligned(25.0f);
				ImGui::Checkbox("##PMVCUseOffsetSettings", &_modifiedProjectModel._pmvcUseOffset);
				ImGui::SameLine();
			}
			}
			ImGui::EndDisabled();
