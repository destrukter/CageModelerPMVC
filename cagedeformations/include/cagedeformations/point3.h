...
public:
    ...
    {
        return point3< T >( std::min< T >( p2[0] , p[0] ),
                            std::min< T >( p2[1] , p[1] ),
                            std::min< T >( p2[2] , p[2] ) );
    }
    ...
    {
        return point3< T >( std::max< T >( p2[0] , p[0] ),
                            std::max< T >( p2[1] , p[1] ),
                            std::max< T >( p2[2] , p[2] ) );
    }
    */
    
    // NOTE: some stupid compilers (won't point the finger...) won't like "min". So I had to replace it with "Min"...
    template< class point_t1 , class point_t2 >
    inline static point3< T > Min ( const point_t1  & p , const point_t2  & p2 ) // some stupid compilers wont allow "min"
    {
        return point3< T >( std::min< T >( (T)(p2[0]) , (T)(p[0]) ),
                            std::min< T >( (T)(p2[1]) , (T)(p[1]) ),
                            std::min< T >( (T)(p2[2]) , (T)(p[2]) ) );
    }

    template< class point_t1 , class point_t2 >
    inline static point3< T > Max ( const point_t1  & p , const point_t2  & p2 ) // some stupid compilers wont allow "max"
    {
        return point3< T >( std::max< T >( (T)(p2[0]) , (T)(p[0]) ),
                            std::max< T >( (T)(p2[1]) , (T)(p[1]) ),
                            std::max< T >( (T)(p2[2]) , (T)(p[2]) ) );
    }
    ...
    */
    ...
    point3<T> slerp( point3<T> const& p0, point3<T> const& p1, double t) {
        ...
        double sin_1 = sin(angle);

        return (sin_1_minus_t / sin_1) * p0 + (sin_t / sin_1) * p1;
    }
...
class NDirectionalBS{
    ...
    void updateIntervals()
    {
        ...
        for( unsigned int i = 0 ; i < ndirs ; ++i )
        {
            ...
            minInterval = std::min< double >( minInterval , interval );
            maxInterval = std::max< double >( maxInterval , interval );
        }
...
public:
    ...
    point3< T > center() const { return (bb + BB) / 2; }
};
